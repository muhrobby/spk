...
import { criteriaWeights, db, client } from "../index";
  ...
  await db.transaction(async (tx) => {
    for (const criteria of DEFAULT_CRITERIA_WEIGHTS) {
      const existing = await tx
        .select({ id: criteriaWeights.id })
        .from(criteriaWeights)
        .where(eq(criteriaWeights.kode, criteria.kode))
        .limit(1);
      ...
      if (existing.length > 0) {
        ...
        await tx.insert(criteriaWeights).values(criteria);
      }
    }
  });
