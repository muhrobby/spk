...
export type WeightsActionResult = {
  success: boolean;
  message: string;
};

export type GetWeightsResult = {
  success: boolean;
  data?: Array<{
    id: number;
    kode: string;
...
export async function getWeightsAction(): Promise<GetWeightsResult> {
...
export async function updateWeightsAction(input: UpdateWeightsInput): Promise<WeightsActionResult> {
  const session = await getAuthSession();
  if (!session) {
    return {
      success: false,
